...
function App() {
  ...
  const handleMove = (boardIndex: number, cellIndex: number) => {
    ...
    // Check if we're restricted to a specific board
    if (gameState.currentBoard !== null) {
      // If the current board is completed or full, allow playing in any available board
      const isCurrentBoardPlayable = !gameState.winners[gameState.currentBoard] && 
                                   !isBoardFull(gameState.board[gameState.currentBoard]);
      
      if (isCurrentBoardPlayable && boardIndex !== gameState.currentBoard) {
        return;
      }
    }
    ...
    const nextBoard = gameState.winners[cellIndex] || isBoardFull(newBoard[cellIndex])
      ? null
      : cellIndex;
